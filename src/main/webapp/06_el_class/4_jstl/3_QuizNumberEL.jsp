...
<!-- 2. 그 값이 없다면 임의의 수를 만들어 세션에 지정(시작할때 딱 한번만 수행) -->
...
<c:choose>
	...
	<c:when test="${ guess gt answer}">정답보다 큽니다.</c:when>
	<c:when test="${ guess lt answer}">정답보다 작습니다.</c:when>
</c:choose>
</c:if>


<%-- <c:if test="${not (empty guess)}">
	<c:if test="${ guess eq answer}">정답입니다.</c:if>
	<c:if test="${ guess gt answer}">정답보다 큽니다.</c:if>
	<c:if test="${ guess lt answer}">정답보다 작습니다.</c:if>
</c:if> --%>

<!-- <c:choose>
	<c:when test="${empty guess}"></c:when>
	<c:otherwise>
		<c:if test="${guess == answer }">정답입니다!</c:if>
		<c:if test="${guess gt answer}">너무 큰 숫자입니다!</c:if>
		<c:if test="${guess lt answer}">너무 작은 숫자입니다!</c:if>
	</c:otherwise>
</c:choose> -->
